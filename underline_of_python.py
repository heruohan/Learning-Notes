...
#############################4、双前导和双末尾下划线__var__
'''
1.由双下划线前缀和后缀包围的变量不会被Python解释器修改.
例1：
class A:
    def __init__(self):
        self.__bam__=42
...
2.但是，Python保留了有双前导和双末尾下划线的名称，用于特殊用途，例如，__init__()构造函数等等......
3.最好避免在自己的程序中使用以双下划线开头和结尾的名称，以避免与将来Python语言的变化产生冲突.
'''
...
2.除了用作临时变量之外，"_"是大多数Python REPL中的一个特殊变量，它表示由解释器评估的最近一个表达式的结果.
因此，你可以在一个解释器会话中访问先前计算的结果，或者，你是在动态构建多个对象并与它们交互，无需事先给这些对象分配名字.
例2：
>>>20+3
23
>>>_
